...
# شامل: LOGIN_HTML, SETUP_HTML, DASHBOARD_HTML, get_public_page_html()

SETUP_HTML = r"""<!DOCTYPE html>
<html lang="fa" dir="rtl">
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>راه‌اندازی اولیه · RVG Gateway</title>
...
:root{
  ...
  --ok:#3FB27F;--danger:#E0656F;--warn:#D9A13B;
}
[data-theme="light"]{
  ...
  --ok:#2E8F66;--danger:#C7414D;--warn:#B07C1F;
}
html{-webkit-text-size-adjust:100%}
body{font-family:'Vazirmatn',system-ui,sans-serif;background:var(--bg);color:var(--text);min-height:100dvh;display:flex}
.mono{font-family:'JetBrains Mono',ui-monospace,monospace;font-variant-numeric:tabular-nums}

.wrap{flex:1;display:flex;align-items:center;justify-content:center;padding:24px 16px;position:relative;overflow:hidden}
.glow{position:absolute;border-radius:50%;filter:blur(90px);opacity:.14;pointer-events:none}
.glow.g1{width:420px;height:420px;background:var(--accent);top:-140px;left:-120px}
.glow.g2{width:360px;height:360px;background:#2C5364;bottom:-120px;right:-100px}

.card{position:relative;width:100%;max-width:520px;background:var(--surface);
  border:1px solid var(--border);border-radius:18px;box-shadow:0 22px 48px -30px rgba(23,38,71,.38);
  animation:rise .5s cubic-bezier(.22,.61,.36,1) both;overflow:hidden}
@keyframes rise{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:none}}

.top{display:flex;align-items:center;gap:11px;padding:16px 22px;border-bottom:1px solid var(--border);background:var(--surface-2)}
.logo{width:36px;height:36px;border-radius:9px;display:flex;align-items:center;justify-content:center;
  background:linear-gradient(135deg,var(--btn-a),var(--btn-b));color:#fff;flex-shrink:0;font-size:17px}
.brand-name{font-size:13.5px;font-weight:700}
.brand-sub{font-size:10.5px;color:var(--dim);margin-top:2px}
.ver{margin-inline-start:auto;font-size:10.5px;color:var(--mid);border:1px solid var(--border);border-radius:6px;padding:3px 8px}

/* ── Steps rail ── */
.rail{display:flex;gap:6px;padding:14px 22px 0}
.step-dot{height:4px;flex:1;border-radius:99px;background:var(--inset);border:1px solid var(--border);
  position:relative;overflow:hidden;transition:border-color .25s}
.step-dot::after{content:'';position:absolute;inset:0;background:linear-gradient(90deg,var(--btn-a),var(--accent-hi));
  transform:scaleX(0);transform-origin:right;transition:transform .45s cubic-bezier(.22,.61,.36,1)}
.step-dot.done::after{transform:scaleX(1)}
.step-dot.on{border-color:var(--accent)}

.body{padding:20px 22px 8px;min-height:298px}
.step{display:none}
.step.on{display:block;animation:fade .32s ease both}
@keyframes fade{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:none}}

.eyebrow{display:inline-flex;align-items:center;gap:7px;font-size:10.5px;color:var(--mid);
  border:1px solid var(--border);border-radius:999px;padding:5px 11px;margin-bottom:14px}
...
.sub{font-size:12.5px;color:var(--mid);line-height:1.85;margin-bottom:18px}

.feat{display:flex;gap:12px;align-items:flex-start;background:var(--inset);border:1px solid var(--border);
  border-radius:12px;padding:12px 14px;margin-bottom:10px}
.feat i{color:var(--accent-hi);font-size:18px;margin-top:2px}
.feat b{display:block;font-size:13px;margin-bottom:2px}
.feat span{font-size:11.5px;color:var(--mid);line-height:1.7}

label{display:block;font-size:11.5px;font-weight:500;color:var(--mid);margin:14px 0 7px;transition:color .18s}
.f-field:focus-within label{color:var(--accent-hi)}
.inp-wrap{position:relative}
input[type=password],input[type=text],input[type=number]{width:100%;padding:13px 40px;border-radius:10px;
...
.ic{position:absolute;right:13px;top:50%;transform:translateY(-50%);color:var(--dim);font-size:16px;pointer-events:none}
.ic-eye{position:absolute;left:9px;top:50%;transform:translateY(-50%);color:var(--dim);font-size:16px;cursor:pointer;
  padding:6px;line-height:0;border-radius:6px;transition:color .16s}
.ic-eye:hover{color:var(--text)}
.grid2{display:grid;grid-template-columns:1fr 1fr;gap:12px}

.meter{height:4px;border-radius:99px;background:var(--inset);margin-top:8px;overflow:hidden;border:1px solid var(--border)}
.meter i{display:block;height:100%;width:0;border-radius:99px;transition:width .3s,background .3s;background:var(--danger)}

.switch-row{display:flex;align-items:flex-start;gap:12px;background:var(--inset);border:1px solid var(--border);
  border-radius:12px;padding:13px 14px;margin-top:16px;cursor:pointer;transition:border-color .16s}
.switch-row:hover{border-color:var(--border-hi)}
.sw-txt{flex:1}
.sw-txt b{display:block;font-size:12.5px;margin-bottom:2px}
.sw-txt span{font-size:11px;color:var(--mid);line-height:1.65}
.switch{position:relative;width:38px;height:21px;flex-shrink:0;margin-top:2px}
.switch input{opacity:0;width:0;height:0}
.slider{position:absolute;inset:0;border-radius:99px;background:var(--inset);border:1px solid var(--border);transition:.2s}
.slider::before{content:'';position:absolute;width:15px;height:15px;border-radius:50%;background:var(--dim);
  top:50%;transform:translateY(-50%);right:2px;transition:.2s}
.switch input:checked + .slider{background:var(--accent-soft);border-color:var(--accent)}
.switch input:checked + .slider::before{background:var(--accent-hi);transform:translate(-100%,-50%)}

.err{display:none;align-items:center;gap:8px;background:var(--inset);border:1px solid var(--danger);
  border-right:3px solid var(--danger);border-radius:9px;padding:10px 12px;margin:0 22px 6px;
  font-size:12.5px;color:var(--danger)}
.err.show{display:flex}

.actions{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:16px 22px}
.btn{display:inline-flex;align-items:center;justify-content:center;gap:8px;border:none;cursor:pointer;
  font-family:inherit;font-weight:600;font-size:13.5px;color:#fff;border-radius:11px;padding:12px 26px;
  background:linear-gradient(135deg,var(--btn-a),var(--btn-b));box-shadow:0 8px 20px -10px var(--btn-ring);
  transition:filter .16s,transform .12s,box-shadow .16s}
.btn:hover{filter:brightness(1.09)}
.btn:active{transform:translateY(1px)}
.btn:disabled{opacity:.55;cursor:not-allowed;filter:none}
.btn.ghost{background:transparent;color:var(--mid);border:1px solid var(--border);box-shadow:none}
.btn.ghost:hover{color:var(--text);border-color:var(--border-hi)}
.spin{animation:spin 1s linear infinite}
@keyframes spin{to{transform:rotate(360deg)}}

.done-wrap{text-align:center;padding:26px 0 12px}
.done-ic{width:64px;height:64px;margin:0 auto 16px;border-radius:50%;display:flex;align-items:center;justify-content:center;
  background:rgba(63,178,127,.12);border:1px solid rgba(63,178,127,.35);color:var(--ok);font-size:30px;
  animation:pop .5s cubic-bezier(.34,1.56,.64,1) both .1s}
@keyframes pop{from{transform:scale(.4);opacity:0}to{transform:scale(1);opacity:1}}
.done-urls{margin:18px auto 4px;text-align:right;background:var(--inset);border:1px solid var(--border);
  border-radius:12px;padding:14px;max-width:400px}
.u-row{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:7px 0}
.u-row + .u-row{border-top:1px dashed var(--border)}
.u-lbl{font-size:11.5px;color:var(--mid)}
.u-val{font-family:'JetBrains Mono',monospace;font-size:11.5px;color:var(--accent-hi);direction:ltr;
  max-width:250px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.note{font-size:11.5px;color:var(--warn);line-height:1.8;margin-top:14px;display:flex;gap:8px;align-items:flex-start;text-align:right}
.foot{display:flex;align-items:center;justify-content:space-between;padding:12px 22px;border-top:1px solid var(--border);
  background:var(--surface-2);font-size:11.5px;color:var(--dim)}

@media(max-width:560px){
  .card{border-radius:14px}.body{min-height:270px}
...
<div class="wrap">
  ...
  <div class="card">
    <div class="top">
      ...
      <div>
        <div class="brand-name">RVG Gateway</div>
        <div class="brand-sub">راه‌اندازی اولیه · Self-Hosted</div>
      </div>
      <span class="ver mono" id="ver">SETUP</span>
    </div>

    <div class="rail">
      <div class="step-dot on" id="d1"></div>
    ...
    <div class="body">
      ...
      <section class="step on" id="s1">
        <span class="eyebrow"><span class="dot"></span> خوش آمدید</span>
        <h1>پنل خود را راه‌اندازی کنید</h1>
        <p class="sub">این ابزار چند مرحله‌ای، پنل RVG را روی سرور شخصی شما آماده می‌کند. فقط چند ثانیه طول می‌کشد.</p>
        <div class="feat"><i class="ti ti-key"></i><div><b>رمز عبور مدیر</b><span>دسترسی امن به داشبورد مدیریتی</span></div></div>
        <div class="feat"><i class="ti ti-world"></i><div><b>آدرس عمومی</b><span>دامنه یا IP سرور برای ساخت لینک‌های اشتراک</span></div></div>
        <div class="feat"><i class="ti ti-plug-connected"></i><div><b>پورت سرویس</b><span>پنل به‌صورت پیش‌فرض روی پورت 8080 اجرا می‌شود</span></div></div>
      </section>

      <!-- Step 2: Admin password -->
      <section class="step" id="s2">
        <span class="eyebrow"><i class="ti ti-key" style="font-size:12px"></i> گام ۱ از ۲</span>
        <h1>رمز عبور مدیر</h1>
        <p class="sub">این رمز برای ورود به داشبورد استفاده می‌شود. حداقل ۶ کاراکتر وارد کنید.</p>
        <label for="pw">رمز عبور</label>
        <div class="inp-wrap f-field">
          <input type="password" id="pw" placeholder="••••••••" minlength="6" required>
          <i class="ti ti-lock ic"></i>
          <i class="ti ti-eye ic-eye" id="eye1" onclick="togglePw('pw','eye1')"></i>
        </div>
        <div class="meter"><i id="meter"></i></div>
        <label for="pw2">تکرار رمز عبور</label>
        <div class="inp-wrap f-field">
          <input type="password" id="pw2" placeholder="••••••••" minlength="6" required>
          <i class="ti ti-checkup ic"></i>
          <i class="ti ti-eye ic-eye" id="eye2" onclick="togglePw('pw2','eye2')"></i>
        </div>
      </section>

      <!-- Step 3: Network -->
      <section class="step" id="s3">
        <span class="eyebrow"><i class="ti ti-world" style="font-size:12px"></i> گام ۲ از ۲</span>
        <h1>تنظیمات شبکه</h1>
        <p class="sub">آدرسی که کاربران برای اتصال استفاده می‌کنند. بعداً هم از داخل پنل قابل تغییر است.</p>
        <label for="host">آدرس عمومی (دامنه یا IP)</label>
        <div class="inp-wrap f-field">
          <input type="text" id="host" placeholder="panel.example.com یا 203.0.113.10" dir="ltr" style="text-align:left">
          <i class="ti ti-server-2 ic"></i>
        </div>
        <div class="grid2">
          <div>
            <label for="aport">پورت اجرای پنل</label>
            <div class="inp-wrap f-field">
              <input type="number" id="aport" min="1" max="65535" dir="ltr" style="text-align:left" oninput="if(!pportTouched) $('pport').value = this.value">
          ...
          <div>
            ...
            <div class="inp-wrap f-field">
              <input type="number" id="pport" min="1" max="65535" dir="ltr" style="text-align:left" oninput="pportTouched=true">
              <i class="ti ti-lock ic"></i>
            </div>
          </div>
        </div>
        <div class="cl" style="margin-top:8px"><i class="ti ti-info-circle"></i><span>پنل خودش مستقیم روی همین پورت با یک گواهی TLS (self-signed) سرو می‌کنه — برای همین این دو پورت باید یکی باشن، مگر این‌که خودت پشت یه ریورس‌پروکسی با گواهی واقعی (مثلاً Let's Encrypt) باشی و بدونی چیکار می‌کنی.</span></div>
        <label class="switch-row">
          ...
          <span class="switch"><input type="checkbox" id="phone"><span class="slider"></span></span>
        </label>
      </section>
      ...
      <section class="step" id="s4">
        <div class="done-wrap">
          <div class="done-ic"><i class="ti ti-check"></i></div>
          <h1>همه چیز آماده است!</h1>
          <p class="sub">پنل با موفقیت پیکربندی شد.</p>
          <div class="done-urls">
            <div class="u-row"><span class="u-lbl">آدرس داشبورد</span><span class="u-val" id="final-url">—</span></div>
            <div class="u-row"><span class="u-lbl">آدرس عمومی لینک‌ها</span><span class="u-val" id="final-host">—</span></div>
          </div>
          <div class="note" id="restart-note" style="display:none">
            <i class="ti ti-info-circle" style="flex-shrink:0;margin-top:2px"></i>
            <span>چون پورت پنل را تغییر دادید، سرویس نیاز به ری‌استارت دارد:<br>
            <span class="mono" dir="ltr">sudo rvg restart</span></span>
          </div>
        </div>
      </section>
    </div>
    ...
    <div class="actions">
      <button type="button" class="btn ghost" id="back" style="visibility:hidden"><i class="ti ti-arrow-right"></i> قبلی</button>
      <button type="button" class="btn" id="next">ادامه <i class="ti ti-arrow-left"></i></button>
    </div>
    </form>

    <div class="foot">
      <span>نصب روی سرور اختصاصی شما</span>
      <span>RVG Gateway · Self-Hosted Edition</span>
    </div>
  </div>
</div>
...
let step = 1;
const LAST = 3;
let pportTouched = false;

function togglePw(inpId, eyeId){
  const pw = $(inpId), eye = $(eyeId);
...
/* theme */
(function(){
  const isDark = localStorage.getItem('rvg-theme') !== 'light';
...
$('pw').addEventListener('input', e => {
  ...
  const m = $('meter');
  m.style.width = pct + '%';
  m.style.background = pct < 40 ? 'var(--danger)' : pct < 70 ? 'var(--warn)' : 'var(--ok)';
});

function go(n){
  $('err').classList.remove('show');
  document.querySelectorAll('.step').forEach((el,i)=>el.classList.toggle('on', i === n-1));
  document.querySelectorAll('.step-dot').forEach((el,i)=>{
    el.classList.toggle('on', i === n-1);
    el.classList.toggle('done', i < n-1 || n > LAST);
  });
  ...
  step = n;
  if (n === 3 && !$('aport').value) { $('aport').value = CURRENT_PORT; $('pport').value = CURRENT_PORT; }
}

function validate(){
  if (step === 2){
    if (($('pw').value || '').length < 6){ return 'رمز عبور باید حداقل ۶ کاراکتر باشد.'; }
    if ($('pw').value !== $('pw2').value){ return 'تکرار رمز عبور مطابقت ندارد.'; }
  }
  if (step === 3){
...
$('next').addEventListener('click', async () => {
  const bad = validate();
  if (bad){ $('err-text').textContent = bad; $('err').classList.add('show'); return; }
  if (step < LAST){ go(step + 1); return; }

  const btn = $('next');
  btn.disabled = true;
  btn.innerHTML = '<i class="ti ti-loader-2 spin"></i> در حال اعمال...';
  try{
    ...
    btn.disabled = false;
    btn.innerHTML = '<i class="ti ti-player-play"></i> پایان راه‌اندازی';
  }
});
    ...
    <div class="cm-body">
      ...
      <div class="cm-section">
        ...
        <div class="cm-dd open" id="dd-base">
          ...
          <div class="cm-dd-panel"><div class="cm-dd-panel-inner"><div class="cm-dd-list">
            ...
            <div class="cm-opt" data-base="reality" onclick="cmSelectBase('reality',this)">
              <div class="cm-opt-radio"></div>
              <div class="cm-opt-icon"><i class="ti ti-shield-check-filled"></i></div>
              <div class="cm-opt-text"><div class="cm-opt-title">VLESS · REALITY</div><div class="cm-opt-desc">TLS واقعی + بدون نیاز به دامنه؛ اجرا با باینری رسمی sing-box</div></div>
              <span class="cm-opt-tag" style="background:var(--green-bg);color:var(--green-t)">جدید</span>
            </div>
            <div class="cm-opt" data-base="trojan-reality" onclick="cmSelectBase('trojan-reality',this)">
              <div class="cm-opt-radio"></div>
              <div class="cm-opt-icon"><i class="ti ti-shield-check-filled"></i></div>
              <div class="cm-opt-text"><div class="cm-opt-title">Trojan · REALITY</div><div class="cm-opt-desc">همون REALITY ولی روی Trojan</div></div>
              <span class="cm-opt-tag" style="background:var(--green-bg);color:var(--green-t)">جدید</span>
            </div>
            <div class="cm-opt" data-base="shadowsocks-xray" onclick="cmSelectBase('shadowsocks-xray',this)">
              <div class="cm-opt-radio"></div>
              <div class="cm-opt-icon"><i class="ti ti-shield-lock-filled"></i></div>
              <div class="cm-opt-text"><div class="cm-opt-title">Shadowsocks · Native</div><div class="cm-opt-desc">پورت اختصاصی، مستقیم روی sing-box (نه WS)</div></div>
              <span class="cm-opt-tag" style="background:var(--green-bg);color:var(--green-t)">جدید</span>
            </div>
            <div class="cm-opt" data-base="hysteria2" onclick="cmSelectBase('hysteria2',this)">
              <div class="cm-opt-radio"></div>
              <div class="cm-opt-icon"><i class="ti ti-bolt"></i></div>
              <div class="cm-opt-text"><div class="cm-opt-title">Hysteria2</div><div class="cm-opt-desc">مبتنی بر QUIC/UDP — عالی برای شبکه‌های ضعیف/پرتاخیر</div></div>
              <span class="cm-opt-tag" style="background:var(--green-bg);color:var(--green-t)">جدید</span>
            </div>
            <div class="cm-opt" data-base="wireguard" onclick="cmSelectBase('wireguard',this)">
              <div class="cm-opt-radio"></div>
              <div class="cm-opt-icon"><i class="ti ti-git-branch"></i></div>
              <div class="cm-opt-text"><div class="cm-opt-title">WireGuard</div><div class="cm-opt-desc">VPN کلاسیک؛ کانفیگ کامل با کلاینت رسمی WireGuard کار می‌کنه</div></div>
              <span class="cm-opt-tag" style="background:var(--green-bg);color:var(--green-t)">جدید</span>
            </div>
          </div></div></div>
        </div>
      ...
      <div class="cm-section" id="mtproto-port-field" style="display:none;margin-bottom:0">
        ...
        <div style="font-size:11px;opacity:.7;margin:-4px 0 10px;line-height:1.7">
          اگر می‌خواهی آدرس/پورت عمومی متفاوتی از پورت داخلی معرفی شود (مثلاً پشت یک ریورس‌پروکسی یا NAT)، اینجا وارد کن
          (همانی که به پورت داخلی بالا map شده). در حالت معمول خالی بگذار تا خودکار همان IP سرور و پورت داخلی استفاده شود.
        </div>
  ...
  <div class="modal-v2 cm-modal" style="max-width:500px">
    ...
    <div class="cm-body">
      ...
      <div id="zp-step-building" style="display:none;text-align:center;padding:14px 0">
        ...
        <div style="color:var(--t3);font-size:12px;margin-top:4px">سرور SOCKS5 روی یک پورت آزاد سرور بالا می‌آید</div>
      </div>
...
<section class="pg on" id="pg-overview">
  ...
  <div class="card" style="margin-top:16px">
    <div class="card-title" style="justify-content:space-between;display:flex;align-items:center">
      <span><i class="ti ti-terminal-2"></i> لاگ زنده‌ی پنل</span>
      <span style="display:flex;align-items:center;gap:8px">
        <label style="font-size:11px;color:var(--t3);display:flex;align-items:center;gap:4px;cursor:pointer">
          <input type="checkbox" id="live-log-autoscroll" checked style="width:14px;height:14px;cursor:pointer"> اسکرول خودکار
        </label>
        <button class="btn btn-g btn-sm" onclick="loadLiveLogs()" title="رفرش دستی"><i class="ti ti-refresh"></i></button>
      </span>
    </div>
    <div id="live-log-box" style="max-height:320px;overflow-y:auto;font-family:monospace;font-size:11.5px;line-height:1.9;background:var(--bg2);border-radius:10px;padding:10px;direction:ltr;text-align:left">
      <div style="color:var(--t3);direction:rtl;text-align:center;padding:20px">در حال دریافت لاگ‌ها...</div>
    </div>
  </div>
  <div class="dash-footer">
    <span class="df-text">codebox RVG Gateway v9.2 · Self-hosted · 2026</span>
    ...
    <div class="srv-panel">
      <div class="srv-hero">
        ...
        <div class="srv-hero-text">
          ...
          <div class="srv-hero-sub"><span class="dot dg pulse"></span> آنلاین · Self-hosted</div>
        </div>
      </div>
      <div class="srv-tiles">
...
function protoBadge(p){
  const m={
    ...
    'vless-reality':['VLESS · REALITY','pc-ss'],
    'trojan-reality':['Trojan · REALITY','pc-ss'],
    'shadowsocks-xray':['Shadowsocks · Native (xray)','pc-ss'],
    'hysteria2':['Hysteria2','pc-ss'],
    'wireguard':['WireGuard','pc-ss'],
  };
...
async function fetchStats(){
  try{
    ...
    document.getElementById('uptime-badge').textContent='Self-hosted · '+d.uptime;
...
const LOG_LEVEL_COLOR = {err:'var(--red-t)', warn:'var(--amber-t)', ok:'var(--green-t)', info:'var(--t2)'};
const LOG_SOURCE_LABEL = {
  system: 'سیستم', 'http-error': 'خطای HTTP', 'sing-box': 'sing-box',
};
function logSourceLabel(src){
  if (LOG_SOURCE_LABEL[src]) return LOG_SOURCE_LABEL[src];
  if (src && src.startsWith('mtproto:')) return 'MTProto · ' + src.slice(8);
  return src || '—';
}
async function loadLiveLogs(){
  ...
  if (!box) return;
  try{
    const r = await authF('/api/live-logs'), d = await r.json();
    const logs = d.logs || [];
    const wasAtBottom = box.scrollHeight - box.scrollTop - box.clientHeight < 40;
    if (!logs.length){
      box.innerHTML = '<div style="color:var(--t3);direction:rtl;text-align:center;padding:20px">هنوز لاگی ثبت نشده</div>';
      return;
    }
    box.innerHTML = logs.map(l => {
      const color = LOG_LEVEL_COLOR[l.level] || 'var(--t2)';
      const t = l.time ? new Date(l.time).toLocaleTimeString('fa-IR') : '';
      return `<div style="margin-bottom:2px"><span style="color:var(--t3)">${t}</span> <span style="color:${color};font-weight:700">[${esc(logSourceLabel(l.source))}]</span> <span style="color:var(--t1)">${esc(l.message||'')}</span></div>`;
    }).join('');
    if (document.getElementById('live-log-autoscroll')?.checked && wasAtBottom){
      box.scrollTop = box.scrollHeight;
    }
  }catch(e){
    box.innerHTML = '<div style="color:var(--red-t);direction:rtl;text-align:center;padding:20px">خطا در دریافت لاگ‌ها</div>';
  }
}
...
const BASE_INFO = {
  ...
  reality:  { icon:'ti-shield-check-filled', title:'VLESS · REALITY', desc:'TLS واقعی + بدون نیاز به دامنه؛ اجرا با باینری رسمی sing-box' },
  'trojan-reality': { icon:'ti-shield-check-filled', title:'Trojan · REALITY', desc:'همون REALITY ولی روی Trojan' },
  'shadowsocks-xray': { icon:'ti-shield-lock-filled', title:'Shadowsocks · Native', desc:'پورت اختصاصی، مستقیم روی sing-box' },
  hysteria2: { icon:'ti-bolt', title:'Hysteria2', desc:'مبتنی بر QUIC/UDP — عالی برای شبکه‌های ضعیف' },
  wireguard: { icon:'ti-git-branch', title:'WireGuard', desc:'VPN کلاسیک با کلاینت رسمی WireGuard' },
};
...
function cmSelectBase(val, el){
  ...
  if (val === 'telproxy') {
    document.getElementById('reality-dest-field').style.display = 'none';
    document.getElementById('manual-port-field').style.display = 'none';
  ...
  } else if (val === 'reality' || val === 'trojan-reality') {
    cmTransport = 'tcp';
    streamSection.style.display = 'none';
    normalNote.style.display = 'none';
    mtNote.style.display = 'none';
    portField.style.display = 'none';
    if (ssField) ssField.style.display = 'none';
    document.getElementById('reality-dest-field').style.display = 'block';
    document.getElementById('manual-port-field').style.display = 'block';
    const isTr = val === 'trojan-reality';
    document.getElementById('cm-head-title').textContent = isTr ? 'ساخت کانفیگ Trojan · REALITY' : 'ساخت کانفیگ VLESS · REALITY';
    document.getElementById('cm-head-sub').textContent = 'TLS واقعی، بدون نیاز به دامنه — با باینری رسمی sing-box';
    document.getElementById('cm-submit-text').textContent = 'ساخت کانفیگ';
    document.getElementById('cm-head-icon').innerHTML = '<i class="ti ti-shield-check-filled"></i>';
  } else if (val === 'shadowsocks-xray' || val === 'hysteria2' || val === 'wireguard') {
    cmTransport = 'tcp';
    streamSection.style.display = 'none';
    normalNote.style.display = 'none';
    mtNote.style.display = 'none';
    ...
    document.getElementById('reality-dest-field').style.display = 'none';
    document.getElementById('manual-port-field').style.display = 'block';
    const titles = {'shadowsocks-xray':'ساخت کانفیگ Shadowsocks (Native)','hysteria2':'ساخت کانفیگ Hysteria2','wireguard':'ساخت کانفیگ WireGuard'};
    document.getElementById('cm-head-title').textContent = titles[val];
    document.getElementById('cm-head-sub').textContent = 'روی یک پورت اختصاصی، کلید/پسورد خودکار ساخته می‌شه';
    document.getElementById('cm-submit-text').textContent = 'ساخت کانفیگ';
    document.getElementById('cm-head-icon').innerHTML = '<i class="ti ti-bolt"></i>';
  } else {
    document.getElementById('reality-dest-field').style.display = 'none';
    document.getElementById('manual-port-field').style.display = 'none';
...
function cmApplyProto(){
  ...
  if (cmBase === 'trojan-reality') {
    document.getElementById('nl-proto').value = 'trojan-reality';
    return;
  }
  if (cmBase === 'shadowsocks-xray') {
    document.getElementById('nl-proto').value = 'shadowsocks-xray';
    return;
  }
  if (cmBase === 'hysteria2') {
  ...
  if (cmBase === 'wireguard') {
    document.getElementById('nl-proto').value = 'wireguard';
    return;
  }
...
async function createLink(){
  ...
  const isReality = protocol === 'vless-reality' || protocol === 'trojan-reality';
  const reality_dest = isReality ? (document.getElementById('nl-reality-dest').value.trim() || 'www.microsoft.com:443') : null;
  const isXcore = isReality || protocol === 'shadowsocks-xray' || protocol === 'hysteria2' || protocol === 'wireguard';
  const manual_port = isXcore ? (document.getElementById('nl-manual-port').value || null) : null;
  try{
    const url = nodeId ? ('/api/nodes/'+nodeId+'/links') : '/api/links';
    const r=await authF(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({label,limit_value:val||0,limit_unit:unit,expires_days:exp||0,note,sub_id,protocol,mtproto_port,mtproto_domain,mtproto_public_host,mtproto_public_port,alpn,fingerprint,ss_cipher,reality_dest,manual_port})});
    if(!r.ok){
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  loadLiveLogs();
  setInterval(() => { if (!document.hidden && document.getElementById('pg-overview').classList.contains('on')) loadLiveLogs(); }, 4000);
...
async function zpDelete(){
  if(!confirm('پروکسی Zeus حذف شود؟')) return;
  try{
