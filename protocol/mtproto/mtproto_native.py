...
def _install_build_deps():
    """نصب پیش‌نیازهای کامپایل MTProxy روی هر توزیع لینوکسی:
    Debian/Ubuntu ← apt | AlmaLinux/Rocky/RHEL/Fedora ← dnf/yum."""
    if shutil.which("gcc") and shutil.which("make"):
        return  # قبلاً نصب است
    try:
        if shutil.which("apt-get"):
            cmd = ("apt-get update -qq && apt-get install -y -qq "
                   "build-essential libssl-dev zlib1g-dev git curl")
        elif shutil.which("dnf"):
            cmd = "dnf install -y gcc make openssl-devel zlib-devel git curl"
        elif shutil.which("yum"):
            cmd = "yum install -y gcc make openssl-devel zlib-devel git curl"
        elif shutil.which("zypper"):
            cmd = "zypper install -y gcc make libopenssl-devel zlib-devel git curl"
        elif shutil.which("pacman"):
            cmd = "pacman -Sy --noconfirm gcc make openssl zlib git curl"
        else:
            logger.warning("MTP: package manager شناسایی نشد — فرض می‌شود ابزار build از قبل نصب هستند.")
            return
        r = subprocess.run(["bash", "-c", f"{cmd} >/dev/null 2>&1"],
                           timeout=300, capture_output=True)
        if r.returncode != 0:
            logger.warning("MTP: نصب build-deps کامل موفق نبود (ممکنه از قبل نصب باشن)")
    except Exception as exc:
        logger.warning(f"MTP: نصب build-deps با خطا مواجه شد (ممکنه از قبل نصب باشن): {exc}")
...
async def ensure_binary() -> bool:
    ...
    async with _build_lock:
        ...
        # پکیج‌های لازم برای build (فقط یک‌بار روی هر سرور) — پشتیبانی از همه‌ی
        # توزیع‌های اصلی لینوکس: Debian/Ubuntu (apt)، AlmaLinux/Rocky/RHEL/Fedora (dnf/yum)
        _install_build_deps()
