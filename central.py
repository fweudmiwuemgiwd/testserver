...
# در نسخه‌ی self-hosted این ارتباط به‌صورت پیش‌فرض «خاموش» است (حریم خصوصی:
# هیچ اطلاعاتی از سرور شما بیرون نمی‌رود). فقط اگر ادمین هنگام Setup یا با
# متغیر محیطی RVG_PHONE_HOME=1 آن را فعال کند، ثبت/heartbeat/اعلان‌ها فعال می‌شوند.
import os
import asyncio
import httpx
...
# main.py هنگام startup این فلگ را از CONFIG["phone_home"] ست می‌کند.
PHONE_HOME = os.environ.get("RVG_PHONE_HOME", "").strip().lower() in ("1", "true", "yes")


async def register_instance():
    if not CENTRAL_URL or not PHONE_HOME:
        return
...
async def report_announcement_views(ids: list[str]):
    ...
    if not CENTRAL_URL or not PHONE_HOME or not ids:
        return
    from main import get_host
    try:
        async with httpx.AsyncClient(timeout=10) as c:
...
async def send_support_message(body: str) -> dict:
    if not CENTRAL_URL or not PHONE_HOME:
        return {"ok": False, "blocked": False, "error": "Phone-Home غیرفعال است"}
    from main import get_host
    try:
        async with httpx.AsyncClient(timeout=10) as c:
